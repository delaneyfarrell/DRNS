...
!                 print *, 'ITER, omega_equator_h', omega_equator_h
                 
                ! ************omega_h becomes a matrix for DR***********
                !*******************************************************
                !
                !------------SOLVING FOR OMG_E AND OMG_C----------------
...
!--------------------------------------------------------------------------
      ...
      double precision function deriv_sm(f,s,m)
       ...
       !-----------------------------------------------------------------------------------------------------------------
           ...
           DOUBLE PRECISION ::  xn, fn, dfn, dxn, xnn, ffn
           DOUBLE PRECISION ::  sq
...
!             print *, 'fcn',  (SQ(xn - omega_equator_h)* &
!             exp(-2.0*SQ(r_e)*rho_equator_h)), r_e
!            print *, 'fcn', xn,fn, omega_equator_h
...
!--------------------------------------------------------------------------
      ...
      FUNCTION NEWTsol(fcnab,xnt1,xnt2,xacc)
      IMPLICIT NONE
      DOUBLE PRECISION :: NEWTsol, xnt1, xnt2, xacc
      EXTERNAL fcnab
...
!-----------------------------------------------------------------------------------------------------
      ...
      !-----------------------------------------------------------------------------------------------------------------
           ...
           DOUBLE PRECISION ::   fnn, dfnn, dxn, xnn, ffnn
           DOUBLE PRECISION ::  sq
...
!--------------------------------------------------------------------------
      ...
      FUNCTION NEWTsol2(fcn2,xnt12,xnt22,xacc2)
      IMPLICIT NONE
      DOUBLE PRECISION :: NEWTsol2, xnt12, xnt22, xacc2
      EXTERNAL fcn2
...
!-----------------------------------------------------------------------------------------------------
        ...
        IF(fl.eq.0.0d0) THEN
          rtsafe = xnt12
          RETURN
        ELSEIF(fh.EQ.0.0d0) THEN
          rtsafe = xnt22
          RETURN
        ELSEIF(fl.LT.0.0d0) THEN  !SEARCH f(xl) < 0
          xl = xnt12
          xh = xnt22
        ELSE
          xh = xnt12
